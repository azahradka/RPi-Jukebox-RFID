...
# Real ``jukebox.utils.atomic_io`` — Phase 1 fix #2. Imported with the real
# path so podcast state-manager writes use the genuine helper under test.
_PKG_ROOT = Path(__file__).resolve().parents[3] / 'src' / 'jukebox'
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# Import the real atomic_io module via importlib so it's installed under
# sys.modules['jukebox.utils.atomic_io'] before we shadow 'jukebox' itself.
